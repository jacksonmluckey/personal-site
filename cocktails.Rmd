...
# then check with %in%
contains_ingredients <- function(ingredients, df) {
  
  # check if ingredient is included in ingredients_to_filter
  tmp <- df %>%
  ...
  # filter df to only include cocktails included in ingredients (tmp)
  df <- df %>%
...
test <- filter_ingredient("Gin", cocktails)  
```
